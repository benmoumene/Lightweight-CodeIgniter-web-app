	...
	<div class="row clearfix">
	  <div class="col-xs-12">
		...
		<div class="col-md-12 column">
                        <a id="addVente" href="<?php echo base_url() ?>home" role="button" class="btn bg-maroon btn-flat btn-sm"><i class="glyphicon glyphicon-plus"></i> Nouvelle vente</a>&nbsp;
            ...
            <div class="box-body">
                    ...
                    <table class="table">
                        <tr>
                            ...
                            <td colspan="4">
                                <label>Soldé/Non Soldé</label>
                                <select id="soldee" name="soldee" class="form-control input-sm">
                                    <option selected="selected" value=""></option>
                    ...
                    <button style="margin-left: 10px;" class="btn btn-lg btn-default">Annuler</button>
                    <button name="" id="btn_rechercher" class="btn btn-lg btn-default">Rechercher</button>
        <div style="height: 50px;">
            
        </div>
        <div id="example1_container">
          ...
          <tbody>
            <?php $sno = 1; echo count($query->result());?>
            ...
            <td>
              <strong><?=$row->CLI_NOM?></strong>
            </td>
            <td>
              <strong><?php echo $row->CLI_PRENOM ?></strong>
            </td>
            <td style="text-align: right">
              <strong style="color:#0080ff;"><?=$row->VEN_A_PAYER?></strong>
            </td>
            <td style="text-align: right">
              <strong style="color:#46CB18;"><?=$row->VEN_PAYE?></strong>
            </td>
            <td style="text-align: right">
              <strong style="color:red;"><?=$row->VEN_A_PAYER - $row->VEN_PAYE ?></strong>
            </td>
            <td style="text-align: right">
              <strong><?php echo $row->VEN_REMISE ?></strong>
            </td>
            <td>
              <strong><?php echo $row->VEN_CREATED ?></strong>
            </td>

                                                <td>
                                                    <div class="btn-group">
                                                        <a href="<?php echo base_url() ?>home/load_edit_view/<?=$row->VEN_ID?>"  role="button" data-toggle="modal" class="btn btn-default btn-sm" title="Modifier"><i class="glyphicon glyphicon-pencil"></i></a>
                                                        <a class="btn btn-default btn-sm" onclick="toggleReglementPane(<?php echo $row->VEN_ID ?>)" title="Régler"><i class="fa fa-money" ></i></a>
                                                        <a db_id="<?php echo $row->VEN_ID ?>" class="btn btn-default btn-sm print_btn" title="Imprimer" href="<?php echo base_url() ?>home/reportReceiptPdf"><i class="fa fa-print"></i></a>
                                                        <a db_id='<?php echo $row->CLI_ID ?>' onclick="toggleReglementsPane(<?php echo $row->VEN_ID ?>)"  class="btn btn-default btn-sm btn_list" title="Règlements"><i class="fa fa-list"></i></a>
                                                     </div>
          ...
          <div class="box box-primary">
            ...
            <div class="box-body pad table-responsive">
                    ...
                    <table id = "example2" class="table display">
                            <thead>
                            <tr>
                              <th>S. No</th><th>Code</th><th>Numéro facture</th><th>Date</th><th>Montant</th>
                            </tr>
                          </thead>
                          <tbody>
        ...
        var vente_id, jsondata;
          ...
          function toggleReglementsPane(id) {
            ...
            if (x.style.display === "none") {
              y.style.display = "none";
              x.style.display = "block";
              var url = '<?php echo base_url() ?>reglementlist/fetchReglements';
              $.ajax({
                  url:url,
                  type:"POST",
                  data:'id='+vente_id
...
<script>
$("#btn_rechercher").on('click', function(e) {
        e.preventDefault();
        alert($("#reservation").data('daterangepicker').startDate.format('YYYY-MM-DD')+"soldee"+document.getElementById('soldee').value+"client"+document.getElementById('client').value);
        var data = 
              {
                "client": document.getElementById('client').value,
                "soldee":document.getElementById('soldee').value,
                "from":$("#reservation").data('daterangepicker').startDate.format('YYYY-MM-DD'),
                "to":$("#reservation").data('daterangepicker').endDate.format('YYYY-MM-DD')
              };

        $.ajax({
            url: "<?php echo base_url().'reglementlist/fetchData'?>",
            type:"POST",
            contentType:"application/json",
            data:JSON.stringify(data)
        }).done(function(data){
            //alert("outered");
            $('#example1').remove(); // this is my <canvas> element
            $('#example1_container').append('<table id="example1"><table>');
...
$('.btn_regler').on('click', function(e) {
        ...
        jsondata  = [{"REG_VENTE_ID":""+vente_id ,"REG_MONTANT":""+montant}];
        $('.confirmLink').trigger('click'); return false;
        });
        
        var regfun = function(){
            var url = '<?php echo base_url() ?>reglementlist/add_reglement';
