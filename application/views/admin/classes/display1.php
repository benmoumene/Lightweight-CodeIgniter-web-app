<?php
	...
	public function populateClasses($k){
		...
		for($u = 0; $u < 5; $u++){
			for($v = 0; $v < 5; $v++){ // 5 pour collège
				$tableau[$u][$v] = $this->_classes[$k][$pos];//cours
				$pos++;			
			}			
		}
		//var_dump($clsRoom);
	...
	private function createOneDimFromTwo(){
		$pos = 0;
		$tableau = array();
		for($i = 0; $i < count($this->_classe); $i++){
			for($u = 0; $u < count($this->_classes); $u++){
				for($v = 0; $v < 25; $v++){
					$tableau[$pos]= $this->_classes[$u][$v];
					$pos++;
				}			
			}
		}
	...
	private function reshuffleTable(){
		$course_class = $this->createOneDimFromTwo();
		$tableau = array();
		for($u = 0; $u < count($this->_classe); $u++){
			$pos = 0;
			for($v = 0; $v < 25; $v++){
				for($i = $pos; $i < count($course_class); $i++){
					if(is_object($course_class[$i])){
						$classe = $course_class[$i]->getClasse();		
						if(($u+1)==$classe){
							$tableau[$u][$v] = $course_class[$i];// si duration = 2 décaler
							$pos = $i+1;
							break;
						}
					}else{
			...
			for($i=0;$i<5;$i++){
				...
				if(12 < $u){
					for($j=1;$j<6;$j++){ //8 pour lycée
						echo '<td><h3>'.$this->getCourseComponents($this->reshuffleTable()[$u][$i][$j-1],0).'</h3></td>';
					}
				}else{
