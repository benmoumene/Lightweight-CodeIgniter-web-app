...
class Depenselist extends CI_Controller {
    
    public function __construct() {
        parent::__construct();
        $this->is_logged_in();
        $this->load->model('Depenselist_model');
    ...
    public function index(){
       $this->get_pagination();
       $this->db->order_by('DEPE_CREATED'); 
       $data['query'] = $this->db->get('depense', 4, $this->uri->segment(3));
       	   //$this->load->view('admin/view_header');
       $this->load->view('admin/view_depenselist', $data);
    }
...
//              $this->Articlelist_model->load_edit_view($this->input->post('id'));
                ...
                $data['query'] =   $this->db->get_where('depense', array('DEPE_ID'=>$id));
                //$this->load->view('admin/view_header');
                $this->load->view('admin/view_editDepenselist',$data);
    }
            
    public function delete_depot(){
                $this->Depenselist_model->delete_depense($this->input->post('id'));
    }
            
    public function get_pagination(){
            $config['base_url'] = base_url().'depenselist/index/';
            $config['total_rows'] = $this->db->get('depense')->num_rows();
            $config['per_page'] = 4;
            $config['num_links'] = 5;
