...
Grid.mixin({
	...
	eventsToRanges: function(events) {
		var _this = this;
		var eventsById = CategorieventsById(events);
		var ranges = [];
...
function CategorieventsById(events) {
	var eventsById = {};
	var i, event;
...
DayGrid.mixin({
	...
	// Given a flat array of segments, return an array of sub-arrays, Categoried by each segment's row
...
TimeGrid.mixin({
	...
	// Given a flat array of segments, return an array of sub-arrays, Categoried by each segment's col
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (groupChildren.length > 1) {
					Categoriel = $('<div/>');
					if (isOnlyButtons) {
						Categoriel.addClass('fc-button-group');
					}
					Categoriel.append(groupChildren);
					sectionEl.append(Categoriel);
				}
				else {
